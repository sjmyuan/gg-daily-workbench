...
| State lifecycle or state machine (e.g., order status lifecycle, user onboarding states, deployment states) | **State Diagram** (`stateDiagram-v2`) when states/transitions dominate; **Flowchart** when decision logic dominates | States as nodes with trigger-labeled transitions vs. decision diamonds and branches — pick by which the reader must see |
...
**Decision rule**: If the primary question is "who talks to whom and in what order?" → use a sequence diagram. If it is "what decisions are made and what paths exist?" → use a flowchart. For state scenarios, if it is "what states exist and what triggers transitions?" → use a state diagram; if branches and decision rules dominate → use a flowchart. When both questions matter, produce both diagrams (state diagram for the lifecycle, flowchart for the decision logic).
